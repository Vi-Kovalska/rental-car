...
const DetailsCar = () => {
  const { id: idByCar } = useParams();
  const car = useSelector(selectCarById);
  const errorFromThunk = useSelector(selectError);
  const dispatch = useDispatch();
  const [notFound, setNotFound] = useState(false);
  console.log(errorFromThunk, typeof errorFromThunk);
  if (errorFromThunk === 404) {
    setNotFound(true);
  }
  useEffect(() => {
    // const controller = new AbortController();
if (idByCar) {
  dispatch(getCarById({ id: idByCar }))
}
    // if
    // return () => <NotFoundPage />;
      ...
      <div className={s.detailsContainer}>
        ...
        <ul className={s.conditionsList}>
          <li className={s.conditionItem}>
            ...
            </svg> {rentalConditions[0].split(",").pop()}
          </li>
          <li className={s.conditionItem}>
            <svg width='16' height='16'>
              <use href='/icons.svg#check-circle' />
